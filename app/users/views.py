...
def github_login(request):
    client_id = GH_ID
    redirect_uri = f"http://{HOST_URL}/users/login/github/callback"
    return redirect(
    ...
    try:
        ...
        if code is not None:
            ...
            if error is not None:
                raise GithubException("Can't get access token", error)
            else:
            ...
            raise GithubException("Cant' get code")
...
def kakao_login(request):
    client_id = KAKAO_ID
    redirect_uri = f"http://{HOST_URL}/users/login/kakao/callback"
    return redirect(
...
class UpdateProfileView(mixins.LoggedInOnlyView, UpdateView):
    ...
    def get_form(self, form_class=None):
        ...
        form.fields["birthdate"].widget.attrs = {"placeholder": "Birthdate - ex) 2020-01-01"}
        return form
